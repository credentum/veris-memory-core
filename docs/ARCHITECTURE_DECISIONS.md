...
|------|-------|--------|------------|----------------|
| 2025-12-26 | VMC-ADR-016: Adaptive Skill Injection | Proposed | TBD | `VMC-ADR-016 adaptive skill injection expert panel neo kung fu` |
...
### 2025-12-26: VMC-ADR-016 - Adaptive Skill Injection (Proposed)
...
**Evidence from ao-suite-20251226-025445:**
- wp-003 failed 3 times with the same issues
- Skill gap detection worked: identified security knowledge missing
- 4 skills injected with 0.54-0.67 relevance scores
- Model escalated to Opus
- Result: Issues went 6→6→5, never passed
- Persistent failures: nil guards, race conditions, edge cases

**Root Cause Analysis:**

| Problem | Impact |
|---------|--------|
| Skills too abstract | Agent can't translate concepts to code |
| Too many skills at once | Context overload (44k chars), conflicting guidance |
| No skill→issue mapping | Can't measure if right skill was applied |
| Missing anti-patterns | Skills show happy path, not failure modes |
| No feedback loop | Same strategy repeated 3 times |
...
#### Phase 2: Parallel Skill Generation (Next Sprint)

Generate skills in parallel while retries continue.

```
Attempt 1 → FAIL
         ↓
   [Parallel Branch]
   ├→ Attempt 2 runs with existing skills (non-blocking)
   └→ ao-panel generates new skill (async)
         ↓
   If Attempt 2 FAILs AND skill ready:
         ↓
   Validate skill (quick reviewer check)
         ↓
   Attempt 3 with new skill injected
```
...
| Component | Purpose |
|-----------|---------|
| Skill Validation Gate | Generated skill gets mini-review before injection |
| Parallel Generation | Don't block retry on skill authoring |
| Failure Pattern Extraction | Capture specific issue from reviewer feedback |
| Relevance Threshold | Require 0.8+ match, not 0.54 |

---

#### Phase 3: Closed-Loop Learning (Future)

Skills improve over time through success tracking.

```
Every successful skill injection:
  → Store: {failure_pattern, skill_content, success_rate}
  → Index by failure pattern embedding
  → Next similar failure: retrieve this exact skill (0.95+ match)
  → Track: skill effectiveness rate

"Neo learned nil guards. Next agent inherits that knowledge."
```

---

#### ao-panel Skill Generation Contract

When ao-panel generates a skill mid-run, output format:

```yaml
# Auto-generated by ao-panel for failure pattern: nil-guard-bypass
name: "Lua Nil Safety Guards (AO)"
trigger_patterns:
  - "nil == nil"
  - "owner check bypass"
  - "authorization always true"
generated_from:
  packet_id: "ao-suite-20251226-wp-003"
  failure_type: "security"
  reviewer_issue: "Owner initialization crashes on nil ao.env.Process.Owner"
sources:
  - "https://cookbook_ao.g8way.io/guides/security.html"
  - "web search: AO Lua nil guard best practices 2025"
content: |
  ## Problem
  Checking `ao.env.Process.Owner == nil` is dangerous because
  if Owner was never set, this comparison crashes.

  ## Anti-Pattern (DON'T DO THIS)
  ```lua
  if ao.env.Process.Owner == msg.From then -- CRASHES if Owner nil
  ```

  ## Safe Pattern (DO THIS)
  ```lua
  local function getOwner()
    return ao.env.Process.Owner or ao.id  -- Safe fallback
  end

  if getOwner() == msg.From then
  ```

  ## Why This Works
  - `ao.id` is always defined (process address)
  - `or` short-circuits nil safely
...
| System | Integration |
|--------|-------------|
| Trajectory Logging | Capture which skill was applied per attempt |
| Saga Events | Track skill injection as saga event |
| Veris Memory | Store generated skills with failure_pattern embedding |
| ao-panel | Extend with web search + skill generation mode |
| Reviewer | Validate generated skills before injection |
...
| `skill_relevance_score` | Match quality for retrieved vs generated |
| `attempts_before_skill_injection` | When does skill generation trigger? |
...
|-----------|--------|-----------|
| 1 | `repeated_failure_same_issue` | 2 consecutive attempts |
| 2 | `skill_retrieval_relevance_max` | < 0.7 (no good match) |
| 3 | `reviewer_issue_pattern_match` | Recognized failure pattern |
| 4 | `manual_skill_generation_request` | 1 |
...
|--------|---------|--------|
| Skill gap closure rate | 0% (3 attempts, same issues) | 80% by attempt 3 |
...
|----------|-----|-------|-----------|
| 1 | VMC-ADR-016 | Adaptive Skill Injection | Static skills don't close gaps - need on-demand generation |
| 2 | VMC-ADR-012 | Forgetting/Decay Mechanism | Without it, memory degrades at scale |
| 3 | VMC-ADR-013 | Success Pattern Learning | Breaks pessimistic bias |
| 4 | - | Metrics/Validation Dashboard | Must prove value before scaling |
| 5 | VMC-ADR-008 | Episodic Memory (Activate) | Temporal queries are table stakes |
